...
if (formInicio) {
  formInicio.addEventListener("submit", (e) => {
    ...
    if (entradaContrasenaLogin)
      entradaContrasenaLogin.classList.remove("is-invalid");

    const correo = entradaCorreoLogin ? entradaCorreoLogin.value.trim() : "";
    const contrasena = entradaContrasenaLogin
      ? entradaContrasenaLogin.value
      : "";

    if (!correo) {
      if (errorCorreoLogin)
        errorCorreoLogin.textContent = "Ingresa tu correo electrónico.";
    ...
    if (!contrasena) {
      if (errorContrasenaLogin)
        errorContrasenaLogin.textContent = "Ingresa tu contraseña.";
      if (entradaContrasenaLogin)
        entradaContrasenaLogin.classList.add("is-invalid");
      valido = false;
...
if (formRegistro) {
  formRegistro.addEventListener("submit", (e) => {
    ...
    if (entradaContrasenaReg)
      entradaContrasenaReg.classList.remove("is-invalid");
    ...
    if (!nombre) {
      if (errorNombreReg)
        errorNombreReg.textContent = "Ingresa tu nombre completo.";
    ...
    if (!correo) {
      if (errorCorreoReg)
        errorCorreoReg.textContent = "Ingresa tu correo electrónico.";
      if (entradaCorreoReg) entradaCorreoReg.classList.add("is-invalid");
      valido = false;
    } else if (entradaCorreoReg && !entradaCorreoReg.validity.valid) {
      if (errorCorreoReg)
        errorCorreoReg.textContent = "Ingresa un correo válido.";
      if (entradaCorreoReg) entradaCorreoReg.classList.add("is-invalid");
      valido = false;
    }

    if (!contrasena) {
      if (errorContrasenaReg)
        errorContrasenaReg.textContent = "Ingresa una contraseña.";
      if (entradaContrasenaReg)
        entradaContrasenaReg.classList.add("is-invalid");
      valido = false;
    } else if (contrasena.length < 8) {
      if (errorContrasenaReg)
        errorContrasenaReg.textContent =
      ...
      (async () => {
        try {
          ...
          if (error) {
            ...
            if (textoOriginal)
              textoOriginal.textContent = "Crear cuenta gratis";
            botonRegistro.disabled = false;
            return;
          }
          mostrarToast("¡Registrado con exito!");
          window.setTimeout(() => {
            cambiarVista("login");
            if (textoOriginal)
              textoOriginal.textContent = "Crear cuenta gratis";
...
[
  entradaCorreoLogin,
  entradaContrasenaLogin,
  entradaNombreReg,
  entradaCorreoReg,
  entradaContrasenaReg,
].forEach((inp) => {
  if (inp) {
